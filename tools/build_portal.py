...
<div class="sandbox" id="sandbox-lab">
  <header class="sb-chrome">
    <div>
      <p class="kicker">Scratch pad · this tab</p>
      <h1>Sandbox</h1>
    </div>
    <p class="lede"><code>print()</code> writes to the console. Ctrl+Enter (Cmd+Enter) runs the whole file. Python stays in this browser.</p>
  </header>
  <details class="sb-help">
    <summary>How to see output</summary>
    <ol class="sb-guide">
      ...
      <li>Quotes make a string: <code>print("hello")</code>. Several values: <code>print("mid", 7.78)</code>.</li>
      <li>A last line that is just a value (for example <code>1 + 1</code>) is also shown. <code>x = 1</code> stays silent — <code>print(x)</code> if you want to see it.</li>
      <li>Errors appear in the same console. <code>import numpy as np</code> and <code>import pandas as pd</code> work.</li>
    </ol>
  </details>
  <div class="sb-work">
    <div class="sb-ed-wrap">
...
.sb-guide { padding-left: 1.2rem; margin: 0.4rem 0 0.2rem; }
.sb-guide li { font-size: 14px; margin: 0.25rem 0; }
body.sandbox-ide {
  overflow: hidden;
  height: 100%;
  max-height: 100vh;
...
body.sandbox-ide main {
  overflow: hidden;
  min-height: 0;
}
body.sandbox-ide article {
  max-width: none;
  height: 100%;
  min-height: 0;
...
body.sandbox-ide #sandbox.panel.on {
  display: flex;
  flex-direction: column;
  flex: 1;
  min-height: 0;
...
.sandbox h1 { font-size: 1.35rem; margin: 0; letter-spacing: -0.03em; }
.sb-chrome {
  display: flex;
  flex-wrap: wrap;
...
.sb-chrome .kicker { margin: 0 0 4px; }
.sb-chrome .lede { margin: 0; font-size: 14px; max-width: 36rem; }
.sb-help {
  flex: 0 0 auto;
  margin: 0 0 8px;
  padding: 0 0 8px;
...
.sandbox .lab-ed { min-height: 0; height: 100%; resize: none; }
.sandbox .cm-s-blotter.CodeMirror {
  height: 100%;
  border: 0;
}
.sandbox #sb-status { font-size: 13px; color: var(--soft); }
.sandbox #sb-out.pass { box-shadow: inset 3px 0 0 var(--gold); }
.sandbox #sb-out.fail { box-shadow: inset 3px 0 0 var(--stamp); }
.sandbox .lab-bar {
  flex: 0 0 auto;
  margin: 0;
  padding: 8px 10px;
...
@media (max-width: 820px) {
  ...
  main { height: auto; }
  body.sandbox-ide {
    grid-template-rows: minmax(72px, 26vh) minmax(0, 1fr);
    overflow: hidden;
  }
  body.sandbox-ide .blotter {
    height: auto;
    max-height: 100%;
    overflow: auto;
  }
...
function show(id) {
  ...
  document.body.classList.toggle('sandbox-ide', id === 'sandbox');
  if (id === 'sandbox' && window.fitSandboxEditor) {
    requestAnimationFrame(() => requestAnimationFrame(() => window.fitSandboxEditor()));
  }
